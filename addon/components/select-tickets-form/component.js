...
export default Component.extend({
  ...
  productHasBookableDateField: filterBy(
    "productFields",
    "slug",
    "has-bookable-date"
  ),
  ...
  persistedProductBasketItems: filter("persistedBasketItems", function(
    persistedBasketItem
  ) {
    return (
      get(persistedBasketItem, "sku.product.id") === get(this, "product.id")
    );
  }),
  ...
  productFieldsHash: computed("productFields.[]", function() {
  ...
  pricedBasketItems: filter("basketItems", function(basketItem) {
  ...
  minQuantityIsValid: computed(
    ...
    function() {
      return (
        get(this, "totalPricedQuantity") >=
        getWithDefault(this, "productMinQuantity", 0)
      );
    }
  ),
  ...
  availableProductQuantity: computed(
    ...
    function() {
      return (
        get(this, "productMaxQuantity") - get(this, "persistedProductQuantity")
      );
    }
  ),
  ...
  totalPrice: computed("basketItems.@each.{price,quantity}", function() {
  ...
  dateLabel: computed("date", function() {
  ...
  isSelectVisitorsDisabled: computed("requiresDate", "date", function() {
  ...
  skuFields: computed("skuFieldArrays.[]", function() {
    let skuFieldArrays = get(this, "skuFieldArrays");
    return skuFieldArrays.reduce(
      (skuFields, skuFieldArray) =>
        skuFields.pushObjects(skuFieldArray.toArray()),
      []
    );
  }),

  skuValueDates: computed("skuDateValueArrays.[]", function() {
    let skuDateValueArrays = get(this, "skuDateValueArrays");
    return skuDateValueArrays.reduce(
      (dates, skuDateValueArray) => dates.pushObjects(skuDateValueArray),
      []
    );
  }),

  days: computed("displayDate", "dates.[]", function() {
    let dates = get(this, "dates");
    let startDay = get(this, "displayDate")
      .clone()
      .startOf("month")
      .weekday(0);
    let finishDay = get(this, "displayDate")
      .clone()
      .endOf("month")
      .weekday(6);
    let days = [];
  ...
  filteredSkus: computed("skus.[]", "date", function() {
