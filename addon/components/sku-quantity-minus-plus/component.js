...
const { Component, computed, get, getProperties, inject, isEmpty, set } = Ember;
...
export default Component.extend({
  ...
  fieldsHash: computed("fields.[]", function() {
  ...
  lineTotal: computed(
    ...
    function() {
      let totalPrice =
        get(this, "bundledBasketItems").reduce(
          (totalPrice, basketItem) => totalPrice + get(basketItem, "sku.price"),
          0
        ) + get(this, "sku.price");
  ...
  available: computed("maxQuantity", "skuBasketItemQuantity", function() {
  ...
  maxQuantity: computed(
    ...
    function() {
      ...
      if (skuMaxQuantity > 0) {
        return Math.min(
          get(this, "productMaxQuantity"),
          get(this, "sku.stockQuantity"),
          skuMaxQuantity
        );
      } else {
        return Math.min(
  ...
  actions: {
    increment() {
      ...
      const quantity = get(basketItem, "quantity");
      const minQuantity = get(this, "skuMinQuantity");
      ...
      set(basketItem, "quantity", Math.max(quantity + 1, minQuantity));
